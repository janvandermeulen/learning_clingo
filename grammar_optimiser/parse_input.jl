...
function parse_tree_to_clingo(input, index, comp = false)
    """
    ...
    # for each of the children: add an edge + a recursive call to this method
    if comp
    ...
    for child in input.children
        ...
        child_nodes, child_edges, index = parse_tree_to_clingo(child, index, comp)
...
function trees_to_clingo(ast, subtrees, i)
    ...
    for (i, s) in enumerate(subtrees)
        ...
        nodes, edges, index = parse_tree_to_clingo(s, index + 1, true)
        output =  output * nodes * "\n%Compression tree edges:" * edges
    end

    #TODO: make the global dictionary with rulenodes
end
